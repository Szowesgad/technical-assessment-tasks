...
const CanvasEditor: React.FC<CanvasEditorProps> = ({
	...
	const handlePointerDown = (event: React.PointerEvent) => {
		...
		const clickedMask = masks.find((mask) =>
			mask.points.some(
				(point) =>
					Math.abs(point.x - clickPoint.x) < 10 &&
					Math.abs(point.y - clickPoint.y) < 10
			)
		);
		...
		if (clickedMask) {
			...
			setDragging(true);
			setDragOffset({
				x: clickPoint.x - clickedMask.points[0].x,
				y: clickPoint.y - clickedMask.points[0].y,
			});
		} else {
			setDrawing(true);
					...
					mask.id === selectedMask
						? {
								...
								points: mask.points.map((point) => ({
									x:
										point.x +
										(movePoint.x - (mask.points[0].x + dragOffset!.x)),
									y:
										point.y +
										(movePoint.y - (mask.points[0].y + dragOffset!.y)),
								})),
						  }
						: mask
				)
